...
def predict(sys,tweet):        
        trained_dir = 'trained_results'
        if not trained_dir.endswith('/'):
                trained_dir += '/'
        test_file = 'data/pre_samples.csv'
        ...
        with tf.Graph().as_default():
                ...
                with sess.as_default():
                        ...
                        def predict_step(x_batch):
                                ...
                                predictions = sess.run([Main.predictions], feed_dict)
                                return predictions
                        ...
                        if y_test is not None:
                                ...
                                cmdd = 'Keras LogDataAnalyzerTweet101 '+sys+' \"'+tweet+'\" '+str(accuracy)
                                #print(cmdd)
                                pipe=os.popen(cmdd)
                                flag = pipe.read()
                                #print(flag)
                                if "es_process" in flag:
                                        initExisting(tweet)
                                if "ps_process" in flag:
                                        initProposed(tweet)
                                if "xs_process" in flag:
                                        initEnhanced(tweet)
                                        plotterAccuracy()
                                        plotterDuration()
