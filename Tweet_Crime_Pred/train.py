...
def train_Main():        
        ...
        with graph.as_default():
                ...
                with sess.as_default():
                        ...
                        grads_and_vars = optimizer.compute_gradients(Main.loss)
                        ...
                        def train_step(x_batch, y_batch):
                                feed_dict = {
                                        Main.input_x: x_batch,
                                        Main.input_y: y_batch,
                                        Main.dropout_keep_prob: params['dropout_keep_prob'],
                                        Main.batch_size: len(x_batch),
                                        Main.pad: np.zeros([len(x_batch), 1, params['embedding_dim'], 1]),
                                        Main.real_len: real_len(x_batch),
                                }
                                _, step, loss, accuracy = sess.run([train_op, global_step, Main.loss, Main.accuracy], feed_dict)

                        def dev_step(x_batch, y_batch):
                                feed_dict = {
                                        Main.input_x: x_batch,
                                        Main.input_y: y_batch,
                                        Main.dropout_keep_prob: 1.0,
                                        Main.batch_size: len(x_batch),
                                        Main.pad: np.zeros([len(x_batch), 1, params['embedding_dim'], 1]),
                                        Main.real_len: real_len(x_batch),
                                }
                                step, loss, accuracy, num_correct, predictions = sess.run(
                                        [global_step, Main.loss, Main.accuracy, Main.num_correct, Main.predictions], feed_dict)
